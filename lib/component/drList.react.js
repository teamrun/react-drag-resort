...
var DrList = React.createClass({
    ...
    componentDidUpdate: function(prevProps, prevState){
        ...
        if( this.state.draggingIndex !== false && this.state.insertPos !== prevState.insertPos ){
    ...
    _mouseMove: function(e){
        if(this.state.draggingIndex !== null){
            ...
            // 检测是否已经拖拽到了视窗的顶或底
            var needScroll = this.checkDragScroll( cursorPageY, newTop );
            if( needScroll !== 0 && needScroll!== undefined ){
                // 容器滚动
                this.el.scrollTop += needScroll*SCROLL_UNIT;
                // 刚计算出来的itemtop要变更一下
                newTop += needScroll*SCROLL_UNIT;
                // 为了下次计算, item start top也要变更一下
                // 当然 到顶就不再变更了
                if( newTop > 0 ){
                    this.draggingData.startItemTop += needScroll*SCROLL_UNIT;
                }
            }
    ...
    // 到顶 返回-1 , 到底 返回 1
    // 没有到边际 返回0
    checkDragScroll: function(curMouseY, newDraggingTop){
        // 鼠标带着元素到边时, 容器要顺势滚动
        // 向上移动
        if( curMouseY < this.draggingData.startCursorTop ){
            if(newDraggingTop <= this.el.scrollTop){
                return -1;
            }
            return 0;
        }
        else if( curMouseY > this.draggingData.startCursorTop ){
            if( newDraggingTop + this.draggingData.elHeight >= this.el.scrollTop + this.el.height ){
                return 1;
            }
            return 0;
        }
